...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ position: "relative", zIndex: 1 }}>
      <div className="shell" style={{ display: "grid", gridTemplateColumns: "232px 1fr", height: "100vh" }}>
        <aside className="sidedesk" style={{ background: "var(--bg2)", borderRight: "1px solid var(--line)", padding: "18px 14px", display: "flex", flexDirection: "column", gap: 4, position: "sticky", top: 0, height: "100vh" }}>
          <Link href="/" style={{ display: "flex", alignItems: "center", gap: 9, fontWeight: 800, color: "var(--ink)", padding: "4px 8px 16px" }}>
            <span style={{ width: 26, height: 26, borderRadius: 8, background: "linear-gradient(135deg,var(--ac),#0a7a65)", display: "grid", placeItems: "center", color: "#fff", fontSize: 13 }}>⚡</span>GrowthTeam
          </Link>
          {ITEMS.map(([label, ico, href]) => (
            <Link key={href} href={href} style={{ display: "flex", alignItems: "center", gap: 11, padding: "10px 12px", borderRadius: 11, fontSize: 13.5, fontWeight: 500, transition: "all .2s", color: on(href) ? "var(--ink)" : "var(--mut)", background: on(href) ? "var(--panel2)" : "transparent", boxShadow: on(href) ? "inset 2px 0 0 var(--ac)" : "none" }}>
              <span>{ico}</span>{label}
      ...
      <style jsx global>{`
        ...
        .mnavbar { display: none; position: fixed; bottom: 0; left: 0; right: 0; z-index: 140; background: #ffffffee; border-top: 1px solid var(--line); backdrop-filter: blur(12px); justify-content: space-around; padding: 8px 4px calc(8px + env(safe-area-inset-bottom)); }
